...
* **Formatting:** Run `dart format .` to ensure consistent code formatting.
* **Fixes:** Run `dart fix --apply` to automatically fix many common errors and help code conform to configured analysis options.
* **Linting:** Run `flutter analyze` or `dart analyze` to catch common issues.
...
Include the package in the `analysis_options.yaml` file. Use the following as a starting point:
...
* **GoRouter Setup:** To use `go_router`, first add it using `flutter pub add go_router`.
...
* **Running Tests:** Run `flutter test` to execute tests.
...
* **Hierarchy and Scale:** Define a set of font sizes for different text elements. Differentiate text effectively using font weights.
...
* **Be brief:** Write concisely. Avoid jargon and acronyms.
